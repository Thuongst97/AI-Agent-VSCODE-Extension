...
const ATLASSIAN_TOOLS = [
    {
        ...
        description: 'Search Jira for issues using a JQL query. Returns a markdown table of matching tickets.',
        inputSchema: {
            type: 'object',
            properties: {
                jql: {
                    type: 'string',
                    description: 'A valid JQL (Jira Query Language) string, e.g. "project = DEV AND status = Open ORDER BY created DESC".',
                },
            },
            required: ['jql'],
        },
    },
    {
        ...
        description: 'Fetch full details (status, description, comments) for a single Jira issue by its key.',
        inputSchema: {
            type: 'object',
            properties: {
                issueKey: {
...
// ─── Tool executor ────────────────────────────────────────────────────────────
async function executeTool(toolCall, api, stream) {
    ...
    try {
        switch (toolCall.name) {
            ...
            case 'create_jira_issue':
                return await api.createJiraIssue(args['projectKey'], args['summary'], args['description']);
...
function createChatHandler(authManager) {
    return async (request, context, stream, token) => {
        ...
        if (!model) {
            stream.markdown('**No compatible Copilot model found.**\n\n' +
        ...
        for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
            if (token.isCancellationRequested)
                break;
            const lmResponse = await model.sendRequest(messages, { tools: ATLASSIAN_TOOLS }, token);
