...
export class SettingsPanel {
  ...
  private static readonly viewType = 'mimiSettings';
  ...
  private async _sendCurrentCredentials(): Promise<void> {
  ...
  private async _verifyCredentials(
    email: string, domain: string, token: string
  ): Promise<void> {
    try {
      const clean = domain.replace(/^https?:\/\//, '').replace(/\/$/, '');
      const svc   = new AtlassianService({ domain: clean, email, pat: token } as AtlassianCredentials);
  ...
  private async _saveCredentials(
    email: string, domain: string, token: string
  ): Promise<void> {
    try {
      const clean = domain.replace(/^https?:\/\//, '').replace(/\/$/, '');
      ...
      vscode.window.showErrorMessage(
        `Mimi Assistant: failed to save — ${err instanceof Error ? err.message : String(err)}`
      );
    }
  }
